...
router.get('/', checkRoles(ROLES.NANNY, ROLES.PARENT), ctrlWrapper(getNannyesController));
  ...
  router.post('/', checkRoles(ROLES.NANNY), upload.single('photo'), validateBody(createNannySchema), ctrlWrapper(createNannyController));

    router.delete('/:nannyId', isValidId, checkRoles(ROLES.NANNY), ctrlWrapper(DeleteNannyController));

    router.put('/:nannyId', isValidId, upload.single('photo'), checkRoles(ROLES.NANNY), validateBody(updateNannySchema), ctrlWrapper(UpsertNannyController));

        router.patch('/:nannyId', isValidId, upload.single('photo'), checkRoles(ROLES.NANNY), validateBody(updateNannySchema), ctrlWrapper(PatchNannyController));
